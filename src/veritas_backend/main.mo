...
shared actor class Veritas() = this {
  ...
  public query func http_request(req : { url : Text; method : Text; headers : [(Text, Text)]; body : Blob }) : async {
    ...
    if (req.url == "/docs" or req.url == "/docs/") {
      // Redirect to GitHub-hosted interactive onboarding guide
      return { status_code = 302; headers = [("Location", "https://github.com/kodydpa-hub/veritas/blob/master/docs/examples/marketplace/onboard.html")]; body = Text.encodeUtf8(""); };
    };
