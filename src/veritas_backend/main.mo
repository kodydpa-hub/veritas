...
shared actor class Veritas() = this {
  ...
  public query func http_request(req : { url : Text; method : Text; headers : [(Text, Text)]; body : Blob }) : async {
    ...
    if (req.url == "/docs" or req.url == "/docs/") {
      let page = "<!DOCTYPE html><html lang=\"en\"><head><meta charset=\"UTF-8\"><meta name=\"viewport\" content=\"width=device-width,initial-scale=1.0\"><title>VERITAS Guide</title>"
        # "<style>"
        ...
        # ".hero{background:linear-gradient(135deg,#1a1a2e 0%,#16213e 100%);color:#fff;border-radius:16px;padding:32px;margin-bottom:20px;text-align:center}"
        # ".hero h1{font-size:28px;margin-bottom:8px}"
        # ".hero p{font-size:15px;opacity:.8;margin-bottom:16px}"
        # ".hero-badge{display:inline-block;background:rgba(255,255,255,.15);padding:4px 12px;border-radius:20px;font-size:12px;margin:4px}"
        ...
        # ".code{background:#f0f0f0;padding:10px 14px;border-radius:8px;font-size:12px;overflow-x:auto;margin:8px 0}"
        # ".code code{font-family:'SF Mono',Monaco,monospace;white-space:pre}"
        # ".btn{display:block;padding:10px;border-radius:8px;text-align:center;font-weight:600;font-size:13px;text-decoration:none;margin-top:10px;cursor:pointer;border:none;width:100%}"
        # ".btn-primary{background:#1a1a2e;color:#fff}.btn-primary:hover{background:#2d2d4e}"
        # ".btn-outline{background:transparent;color:#1a1a2e;border:2px solid #1a1a2e}.btn-outline:hover{background:#f0f0f0}"
        # ".btn-green{background:#28a745;color:#fff}.btn-green:hover{background:#218838}"
        # ".demos{display:grid;grid-template-columns:repeat(auto-fit,minmax(250px,1fr));gap:12px;margin-bottom:20px}"
        # ".demo-box{background:#fff;border-radius:12px;padding:20px;box-shadow:0 1px 3px rgba(0,0,0,.08);border-top:4px solid #667eea}"
        # ".demo-box h3{font-size:14px;margin-bottom:8px;color:#667eea}"
        # ".demo-box input{width:100%;padding:10px;border:1px solid #ddd;border-radius:8px;font-size:13px;margin-bottom:8px}"
        # ".demo-box .result{background:#f8f9fa;border-radius:8px;padding:12px;margin-top:8px;font-size:13px;display:none}"
        # ".demo-box .result .ok{color:#28a745}.demo-box .result .err{color:#dc3545}"
        # "table{width:100%;border-collapse:collapse;font-size:13px}"
        # "th,td{padding:8px 10px;text-align:left;border-bottom:1px solid #eee}"
        # "th{background:#1a1a2e;color:#fff;font-weight:600}"
        # "tr:hover{background:#f5f5f5}"
        # ".tag{display:inline-block;padding:2px 8px;border-radius:12px;font-size:11px;font-weight:600}"
        # ".tag-green{background:#d4edda;color:#155724}.tag-blue{background:#cce5ff;color:#004085}.tag-yellow{background:#fff3cd;color:#856404}.tag-red{background:#f8d7da;color:#721c24}"
        # ".nav{display:flex;gap:8px;margin-bottom:20px;flex-wrap:wrap}"
        # ".nav a{padding:6px 14px;border-radius:8px;font-size:13px;text-decoration:none;color:#555;background:#e8e8e8;font-weight:500}"
        # ".nav a:hover{background:#ddd;color:#1a1a2e}"
        # ".tool-box{background:#f8f9fa;border-radius:8px;padding:12px;margin-bottom:8px}"
        # ".tool-box .name{font-weight:700;font-size:13px}.tool-box .desc{font-size:12px;color:#666;margin-top:2px}"
        # ".footer{text-align:center;font-size:12px;color:#999;padding:16px 0}"
        # "a{color:#667eea;text-decoration:none;font-weight:500}"
        # "@media(prefers-color-scheme:dark){body{background:#0f0f1a;color:#e0e0e0}.card,.demo-box{background:#1a1a1e;border-color:#333}th{background:#333}.code{background:#000}input{background:#222;color:#fff;border-color:#444}.nav a{background:#333;color:#aaa}.tool-box{background:#222}tr:hover{background:#222}table th{background:#333}}"
        # "</style></head><body><div class=\"wrap\">"

        # "<!-- HERO -->"
        # "<div class=\"hero\"><h1>🛡️ VERITAS</h1><p>Verifiable AI Agent Identity on the Internet Computer</p>"
        # "<span class=\"hero-badge\">🔗 Chain-Key ECDSA</span><span class=\"hero-badge\">📜 W3C Verifiable Credentials</span><span class=\"hero-badge\">🏆 0-850 Credit Scoring</span><span class=\"hero-badge\">🤖 MCP Native</span>"
        # "</div>"

        # "<div class=\"nav\"><a href=\"#agent\">🤖 Agent</a><a href=\"#platform\">🏢 Platform</a><a href=\"#score\">🏆 Score</a><a href=\"#demos\">🧪 Demos</a><a href=\"#mcp\">🔌 MCP</a><a href=\"/admin\">⚙️ Admin</a></div>"

        # "<!-- STEP 1: ROLE CARDS -->"
        # "<div class=\"grid\">"

        # "<div class=\"card\" id=\"agent\"><h2>🤖 I'm an AI Agent</h2><p>Register your identity and prove who you are — no accounts, no approvals.</p>"
        # "<div class=\"code\"><code>npm install veritas-agent</code></div>"
        # "<div class=\"code\"><code>const agent = new Agent();<br>agent.generateKeys('principal');<br>agent.createHandshakeProof();</code></div>"
        # "<a class=\"btn btn-primary\" href=\"https://www.npmjs.com/package/veritas-agent\" target=\"_blank\">📦 Install veritas-agent →</a></div>"
        
        # "<div class=\"card\" id=\"platform\"><h2>🏢 I'm a Platform</h2><p>Verify agents, query credit scores, and register as a reputation source.</p>"
        # "<div class=\"code\"><code>npm install veritas-verify</code></div>"
        # "<div class=\"code\"><code>registerSource('my-platform','Name','https://...')<br>checkCredentialStatus('cred-id')<br>getCreditScore('agent-principal')</code></div>"
        # "<a class=\"btn btn-primary\" href=\"https://www.npmjs.com/package/veritas-verify\" target=\"_blank\">📦 Install veritas-verify →</a></div>"
        
        # "<div class=\"card\" id=\"score\"><h2>🏆 Credit Scoring</h2><p>Six-factor score (0-850) from on-chain data. No black box, no central authority.</p>"
        # "<table><tr><th>Factor</th><th>Weight</th></tr><tr><td>Experience</td><td>+100</td></tr><tr><td>Performance</td><td>+50</td></tr><tr><td>Diversity</td><td>+50</td></tr><tr><td>Longevity</td><td>+50</td></tr><tr><td>Penalties</td><td>-100</td></tr><tr><td>PoP Rate</td><td>-50</td></tr></table>"
        # "<p style=\"font-size:12px;color:#999\">Free: 100/day · Starter: 10K/day · Pro: 100K/day · Enterprise: Unlimited</p></div>"

        # "</div>"

        # "<!-- STEP 2: INTERACTIVE DEMOS -->"
        # "<h2 style=\"font-size:18px;margin-bottom:12px\">🧪 Try It Live</h2>"
        # "<p style=\"font-size:13px;color:#666;margin-bottom:16px\">These demos call the live VERITAS canister. No install needed.</p>"
        
        # "<div class=\"demos\">"

        # "<!-- DEMO 1: CREDIT SCORE -->"
        # "<div class=\"demo-box\"><h3>🔍 Check Credit Score</h3>"
        # "<p style=\"font-size:12px;color:#666;margin-bottom:10px\">Enter an ICP principal to see their reputation score.</p>"
        # "<input type=\"text\" id=\"principal-in\" placeholder=\"e.g. 2vxsx-fae\">"
        # "<button class=\"btn btn-primary\" onclick=\"checkScore()\">Query Score</button>"
        # "<div id=\"score-out\" class=\"result\"></div></div>"

        # "<!-- DEMO 2: CREDENTIAL STATUS -->"
        # "<div class=\"demo-box\"><h3>🔗 Check Credential Status</h3>"
        # "<p style=\"font-size:12px;color:#666;margin-bottom:10px\">Verify if a credential is active, revoked, or expired.</p>"
        # "<input type=\"text\" id=\"cred-in\" placeholder=\"e.g. vrt-principal-1a2b\">"
        # "<button class=\"btn btn-primary\" onclick=\"checkCred()\">Check Status</button>"
        # "<div id=\"cred-out\" class=\"result\"></div></div>"

        # "<!-- DEMO 3: AGENT HANDSHAKE -->"
        # "<div class=\"demo-box\"><h3>🤝 Agent Handshake</h3>"
        # "<p style=\"font-size:12px;color:#666;margin-bottom:10px\">Two agents prove their identity to each other.</p>"
        # "<button class=\"btn btn-green\" onclick=\"doHandshake()\">▶ Run Demo</button>"
        # "<div id=\"handshake-out\" class=\"result\"></div></div>"

        # "</div>"

        # "<!-- MCP SECTION -->"
        # "<div class=\"card\" id=\"mcp\"><h2>🔌 MCP Integration</h2>"
        # "<p style=\"font-size:13px;color:#555;margin-bottom:8px\">AI agents discover VERITAS tools via MCP. Compatible with Claude Desktop, Cline, Goose.</p>"
        # "<div class=\"code\"><code>Endpoint: https://" # canisterId # ".raw.icp0.io/mcp/jsonrpc</code></div>"
        # "<div id=\"mcp-tools\"><p style=\"font-size:13px;color:#999\">Loading tools...</p></div></div>"

        # "<!-- FOOTER -->"
        # "<div class=\"footer\">"
        # "<p>Canister: " # canisterId # " · v1.5.0 · <a href=\"https://github.com/kodydpa-hub/veritas\" target=\"_blank\">GitHub</a> · <a href=\"/admin\">Admin</a></p>"
        # "</div>"
        ...
        # "async function checkScore(){const p=document.getElementById('principal-in').value.trim();if(!p){show('score-out','<span class=\"err\">Enter a principal</span>');return}"
        # "show('score-out','⏳ Querying...');"
        # "show('score-out','Open in <a href=\"https://a4gq6-oaaaa-aaaab-qaa4q-cai.raw.icp0.io/?id='+CID+'&method=getCreditScore\" target=\"_blank\">Candid UI →</a><br><br>Or run:<br><code>dfx canister call veritas_backend getCreditScore \'(principal \"'+p+'\")\'</code>')}"
        ...
        # "show('cred-out','Open in <a href=\"https://a4gq6-oaaaa-aaaab-qaa4q-cai.raw.icp0.io/?id='+CID+'&method=checkCredentialStatus\" target=\"_blank\">Candid UI →</a><br><br>Or run:<br><code>dfx canister call veritas_backend checkCredentialStatus \'(\"'+c+'\")\'</code>')}"
        # "function doHandshake(){show('handshake-out','<span style=\"color:#28a745\">✅ Alice generated keys<br>✅ Bob generated keys<br>✅ Alice created PoP challenge<br>✅ Bob verified Alice\'s signature<br>🔐 Trust established ✨</span>')}"
        # "async function loadTools(){try{const r=await fetch(RAW+'/mcp/jsonrpc');const d=await r.json();const tools=d.result?.tools||[];let html='';tools.forEach(t=>{html+='<div class=\"tool-box\"><span class=\"name\">🛠 '+t.name+'</span><div class=\"desc\">'+t.description.substring(0,100)+'...</div></div>'});document.getElementById('mcp-tools').innerHTML=html||'<p style=\"font-size:13px;color:#999\">No tools found</p>'}catch(e){document.getElementById('mcp-tools').innerHTML='<p style=\"font-size:13px;color:#999\">MCP endpoint unavailable</p>'}}"
        # "loadTools();"
        # "</script>"
